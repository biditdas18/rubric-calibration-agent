...
% ── Abstract ──────────────────────────────────────────────────────────────
\begin{abstract}
Annotation quality is the limiting factor in supervised learning pipelines
...
agreement---statistically indistinguishable from random assignment and
insufficient to produce reliable training labels. Manual rubric refinement
does not scale and introduces annotator-specific bias that cannot be audited or
reproduced. We present \textbf{CRUCIBLE}, a multi-agent system that
...
analyses per-model criterion-level disagreement patterns and proposes rubric
revisions; the loop repeats until weighted inter-judge agreement reaches a
target threshold or stagnation is detected. CRUCIBLE was developed to solve
...
produced labels no better than a coin toss. Applied to 60 YouTube transcripts
across three content domains---General Education, Technology \& AI, and Career
\& Self-Improvement---CRUCIBLE reached 96.7\% and 81.7\%
weighted inter-judge agreement on two domains, up from a 53\% pairwise baseline, while
the third domain stagnated at 70.0\%---near the floor of the weighted metric---revealing a
model capability ceiling rather than a successful calibration.
...
misapplied a co-occurrence criterion that no rubric revision could compensate
for. The system also detected and logged a degenerate judge (Gemma 2 9B, producing
...
LLM ensemble. It yielded 53\% pairwise inter-judge agreement---a coin toss.
...
and more fundamentally, manual annotation introduces irreducible personal bias:
an annotator who values career development content intrinsically applies a
systematically different threshold than one who does not. That bias cannot be
audited, disclosed, or corrected---it is baked into every label silently.
...
proposes targeted rubric revisions; the loop repeats until convergence or
stagnation. The resulting domain-specific rubrics are versioned and auditable.
...
The downstream result validates the approach. The rubrics produced by CRUCIBLE were applied by a separate frontier ensemble (GPT-4o mini,
...
disagreement diagnosis and targeted revision. To our knowledge, no prior work
runs a fully automated iterative rubric refinement loop driven solely by
inter-judge agreement signal with no human in the loop.

\item \textbf{The model capability ceiling finding.} The system empirically
distinguishes rubric-fixable disagreement (Technology \& AI: 77\%
$\rightarrow$ 82\% in one iteration) from model-capability-bounded disagreement
(Career \& Self-Improvement: stagnated at 70.0\% across nine iterations). This
...
minutes (incomplete) to $\approx$17 minutes on consumer hardware.

\item \textbf{End-to-end empirical validation.} The calibrated rubrics, applied downstream by a frontier ensemble, produced labels that
trained an SVM classifier achieving F1\,=\,0.871 and recall\,=\,0.964 on the
HIGH-signal class, demonstrating that rubric calibration quality translates to downstream classifier
performance.
...
domain-specific adaptation~\cite{fu2023gptscore}. A persistent finding is
that single-model judges exhibit systematic biases---position bias, verbosity
bias, and self-preference---that reduce
reliability~\cite{shi2024judging}. Multi-model ensembles have been proposed to
...
own rubrics. Both still rely on reference judgments or accuracy signals; CRUCIBLE instead
drives revision purely from inter-judge agreement with no human reference labels.

\subsection{Disagreement Diagnosis}

AraTable~\cite{aratable2025} proposes a self-deliberation mechanism in which a judge re-examines its own
...
\node[pbox={green!60!black}{2.8cm}, right=1.6cm of dec] (conv)
  {\textbf{Converged}\\save rubric \& labels};
...
\node[stobox, below=0.65cm of coord] (stop)
  {\textbf{Stopping:}\;
   ...
   stagnation ($\Delta A\!\leq\!0$ for 3 iter.) \textbar\;
   max 10 iterations};
...
Qwen~2.5~7B (Alibaba). Temperature~0.0 ensures deterministic outputs, isolating
...
\textbf{Stopping.} Loop terminates when: (a)~all domains converged; (b)~all
non-converged domains stagnated ($\Delta A_d \leq 0$ for three consecutive
iterations); or (c)~maximum 10 iterations reached.
...
only domain with fair chance-corrected agreement, and the only one where a rubric revision raised the raw agreement metric. Career \&
...
\checkmark\,=\,converged; $\triangle$\,=\,stagnated.}
...
For Career, the second revision (v2) regressed to 68.4\%, so the best rubric (v1, 70.0\%,
iteration~2) was retained as the system output; agreement never reached the 80\% target.
...
end-to-end validation of the calibrated rubrics: a frontier ensemble applying
them produced labels that train a strong classifier. Full experimental
...
The Career \& Self-Improvement stagnation at 70.0\% demonstrates that a class
of disagreement exists that rubric revision cannot fix: disagreement caused by
...
the coordinator's consistent diagnosis across nine iterations provides a
...
\textbf{Model-specific ceiling.} The Career stagnation was diagnosed as
specific to Qwen~2.5~7B. Future work should evaluate judge replacement as a
recovery strategy.

\textbf{Silver labels.} Calibrated labels retain the biases of the three judge
models. For high-stakes tasks, human adjudication of remaining disagreements is
recommended.
...
CRUCIBLE demonstrates that annotation rubric calibration can be fully
automated through a coordinator-mediated iterative loop driven by inter-judge
agreement signal, with no human in the loop after initialization. The system
was built to solve a concrete problem---53\% coin-toss agreement that made a
binary transcript classification task unlearnable---and raw weighted agreement rose to 81.7\%--96.7\% on two domains, though
...
alignment.
\newblock In \emph{EMNLP 2023}, 2023.
...
H.~Hashemi, J.~Saad-Falcon, B.~Van Durme, and C.~Kedzie.
\newblock {LLM-Rubric}: A multidimensional, calibrated approach to automated
evaluation of natural language texts.
\newblock In \emph{ACL 2024}, pp.\ 8725--8744, 2024.

\bibitem{adarubric2026}
L.~Ding.
\newblock AdaRubric: Task-Adaptive Rubrics for {LLM} Agent Evaluation.
...
S.~H. Bach, D.~Ratner, B.~He, F.~Niu, C.~R{\'e}, and C.~Sa.
\newblock A case for programmatic weak supervision.
\newblock \emph{arXiv:1711.10160}, 2019.
